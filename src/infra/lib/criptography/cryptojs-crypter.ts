...
export class CryptoJSCrypter implements Encrypter, Decrypter {
  async encrypt(payload: Record<string, unknown>): Promise<string> {
    ...
    const encryptValue = CryptoJS.AES.encrypt(payloadString, env.ENCRYPTION_KEY).toString();
    return encryptValue;
  }

  async decrypter(encryptValue: string): Promise<Record<string, unknown>> {
    const decrypted = CryptoJS.AES.decrypt(encryptValue, env.ENCRYPTION_KEY).toString(
      CryptoJS.enc.Utf8,
    );
