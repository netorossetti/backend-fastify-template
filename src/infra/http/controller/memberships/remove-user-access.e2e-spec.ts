...
describe("Membership - RemoveUserAccess (e2e)", () => {
  ...
  test("Deve ser possível remover o acesso de usuário.", async () => {
    ...
    const response = await request(app.server)
      .delete(`/memberships/users/${user2.id.toString()}`)
    ...
    expect(verifyMembership).toBe(null);
  });
});
