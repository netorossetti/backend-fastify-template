...
type RemoveUserAccessUseCaseResponse = Result<NotFoundError | NotAllowedError, {}>;
...
export class RemoveUserAccessUseCase {
  ...
  async execute({
    ...
    const updateMembership = await this.membershipsRepository.findByUserAndTenant(
      removeUser.userId,
      tenant.id.toString(),
    );
    if (!updateMembership) return failure(new NotFoundError("Permisão de acesso não localizada."));
