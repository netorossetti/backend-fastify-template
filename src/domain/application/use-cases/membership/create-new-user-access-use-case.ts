...
export class CreateNewUserAccessUseCase {
  ...
  async execute({
    ...
    if (canCreateUser)
      return failure(
        new NotAllowedError("Usuário não tem permisão necessária para criar um novo usuário."),
      );

    const userExists = await this.usersRepository.findByEmail(createUser.email);
    if (userExists) {
      const membershipExists = await this.membershipsRepository.findByUserAndTenant(
