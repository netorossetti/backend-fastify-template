...
export const jsonZodSchema: z.ZodType<Json> = z.lazy(() =>
  z.union([literalSchema, z.array(jsonZodSchema), z.record(z.string(), jsonZodSchema)]),
);
      ...
      Object.fromEntries(
        jsonObjectSchemas.map((schema) => {
          ...
          switch (schema.type) {
            ...
            case "INTEGER":
              zobejct = z.coerce.number().int().max(limiteSuperior).min(limiteInferior);
              if (schema.nullable) zobejct = zobejct.nullable();
              break;
            ...
            case "MONEY":
              zobejct = z.coerce
                .string()
                .refine((value) => /^R\$\d{1,3}(\.\d{3})*(,\d{2})?$/.test(value), {
                  message:
            ...
            case "OBJECT":
              if (!schema.objectSchemas)
                throw new Error('Conversion failed to load field schemas type "OBJECT".');
